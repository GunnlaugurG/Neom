...
// const game = new Chess('5B2/1nqn4/1pk5/1pb5/8/5B2/8/3b3K b - - 2 1');
const game = new Chess();

class Stockfish extends Component {
	static propTypes = { children: PropTypes.func, playerColor: PropTypes.string, test: PropTypes.bool };
	...
	engineGame = (options) => {
		...
		const setSkillLevel = (level) => {
			...
			uciCmd(`setoption name Skill Level Probability value ${level}`)
			...
			if (line === "uciok") {
				...
				if (match) {
					...
					if (turn !== playerColor) {
						...
						this.setState({ bestMove: [[match[1], match[2]]] })
					}
					uciCmd("eval", evaler);
					//uciCmd("eval");
					...
					// console.log(engineStatus.search, match)
				}
				...
				engineStatus.score !== undefined && this.setState({ engineScore: engineStatus.score })
			}
			// displayStatus();
		};
	...
	render() {
		...
		return this.props.children({ position: fen, onDrop: this.onDrop, bestMove: bestMove, engineStatus: engineScore, returning: this.props.test });
	}
}
